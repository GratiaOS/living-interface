...
export function bootOS() {
  ...
  const kernel = new PresenceKernel(1000).use(consoleAdapter()).use(presencePing(peerId));
  ...
  if (typeof window !== 'undefined') {
    (window as any).gratia = {
      ...
      phase: (phase: Parameters<PresenceKernel['setPhase']>[0]) => kernel.setPhase(phase),
      mood: (mood: Parameters<PresenceKernel['setMood']>[0]) => kernel.setMood(mood),
      transport,
    };
    console.info(`📡 Signal online via ${transport}. Try: gratia.radar(), gratia.whisper("presence")`);
  }
